...
文件名超长、文件超过 50MB 或扩展名不在 Keystone 白名单时，返回 `10406` 包装后的上传失败响应：
...
通知公告不存在时，与 Keystone 一致返回 `200 OK` 业务失败响应，`code=10001`，`msg/message` 形如 `找不到ID为 42 的 通知公告`。
...
通知公告不存在时返回 `200 OK` 业务失败响应，`code=10001`。
...
Keystone 兼容行为：菜单不存在时仍返回 `200 OK` 成功响应，`data` 是字段值为 `null` 的空菜单详情对象。
...
**Keystone 兼容业务失败** `200 OK`

| code | msg |
| --- | --- |
| `10901` | `新增菜单:{} 失败，菜单名称已存在` |
| `10902` | `菜单外链必须以 http(s)://开头` |
| `10906` | `不允许在Iframe和外链跳转类型下创建按钮` |
| `10907` | `只允许在目录类型底下创建子菜单` |
...
| code | msg |
| --- | --- |
| `11001` | `角色名称：{roleName}, 已存在` |
| `11002` | `角色标识：{roleKey}, 已存在` |
...
Keystone 兼容行为：部门不存在时仍返回 `200 OK` 成功响应，`data` 是字段值为 `null` 的空部门详情对象。
...
更新部门。父级不能选择自身，存在同级同名部门时返回 Keystone 兼容业务失败。
...
| code | msg |
| --- | --- |
| `10001` | `找不到ID为 {deptId} 的 部门` |
| `10801` | `部门名称:{deptName}, 已存在` |
| `10802` | `父级部门不能选择自己` |
| `10803` | `子部门还有正在启用的部门，暂时不能停用该部门` |
| `10806` | `该父级部门不存在或已停用` |
...
| --- | --- |
| `10001` | `找不到ID为 {deptId} 的 部门` |
| `10804` | `该部门存在下级部门不允许删除` |
| `10805` | `该部门存在关联的用户不允许删除` |
...
Keystone 兼容行为：岗位不存在时仍返回 `200 OK` 成功响应，`data` 是字段值为 `null` 的空岗位详情对象。
...
| code | msg |
| --- | --- |
| `10701` | `岗位名称:{postName}, 已存在` |
| `10702` | `岗位编号:{postCode}, 已存在` |
...
用户不存在时，与 Keystone 一致仍返回 `200 OK` 成功响应，`user` / `roleId` / `postId` 为空，同时返回可选角色和岗位列表。
...
更新时与 Keystone 一致不按用户名做唯一性阻断；会校验手机号、邮箱和关联实体。

**Keystone 兼容业务失败**

| `code` | `msg/message` |
| --- | --- |
| `10001` | `找不到ID为 {userId} 的 用户` |
| `10508` | `该电话号码已被其他用户占用` |
| `10509` | `该邮件地址已被其他用户占用` |
| `10001` | `找不到ID为 {postId} 的 职位` |
| `10001` | `找不到ID为 {deptId} 的 部门` |
...
用户不存在时返回 `200 OK` 业务失败响应，`code=10001`，`msg/message` 形如 `找不到ID为 2 的 用户`。
...
字典类型不存在时，与 Keystone 一致返回 `200 OK` 业务失败响应，`code=10001`，`msg/message` 形如 `找不到ID为 42 的 字典类型`。
...
字典类型不存在时返回 `200 OK` 业务失败响应，`code=10001`。

---

### DELETE `/api/system/dict/type/{dictId}`

删除字典类型。若该类型下仍存在字典数据，与 Keystone 一致返回 `200 OK` 业务失败响应，`code=11101`，`msg/message=字典类型下存在字典数据，请先删除字典数据`。
...
字典数据不存在时，与 Keystone 一致返回 `200 OK` 业务失败响应，`code=10001`，`msg/message` 形如 `找不到ID为 42 的 字典数据`。
...
字典数据不存在时返回 `200 OK` 业务失败响应，`code=10001`。
